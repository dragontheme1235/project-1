...
?>
<?php
class DGTThemes_DGTYaris_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    {
        return Mage::getStoreConfig('dgtyaris/' . $optionString);
    }
    ...
    {
        if ($storeId)
            return Mage::getStoreConfig('dgtyaris_design', $storeId);
        else
            return Mage::getStoreConfig('dgtyaris_design');
    }
    ...
    {
        return Mage::getStoreConfig('dgtyaris_design/' . $optionString, $storeCode);
    }
    ...
    {
        return Mage::getStoreConfig('dgtyaris_layout/' . $optionString, $storeCode);
    }
    ...
    public function getLabel(Mage_Catalog_Model_Product $product)
    {
        ...
        if ( $this->getCfg("product_labels/sale") && $this->_checkSale($product) ) {
            ...
            $_percent = ($special_price / $price) * 100;
            $_percent_sale = round(100 - $_percent);
            $html .= '<div class="product-sale-label">';
            $html .= '- ' . $_percent_sale. '%';
            $html .= '</div>';
        }
    ...
    public function getAltImgHtml($product, $w, $h, $imgVersion='small_image')
    {
        ...
        if ($gal = $product->getMediaGalleryImages())
        {
            ...
            {
                return
                    '<img class="img-responsive" src="' . Mage::helper('dgtyaris/image')->getImg($product, $w, $h, $imgVersion, $altImg->getFile()) . '" alt="' . $product->getName() . '" />';
            }
        }
    ...
    {
        $config['zoomWidth'] = is_numeric(Mage::getStoreConfig('dgtyaris/product_page/zoom_width'))?Mage::getStoreConfig('dgtyaris/product_page/zoom_width'):'auto';
        $config['zoomHeight'] = is_numeric(Mage::getStoreConfig('dgtyaris/product_page/zoom_height'))?Mage::getStoreConfig('dgtyaris/product_page/zoom_height'):'auto';
        $config['position'] = Mage::getStoreConfig('dgtyaris/product_page/zoom_position')?Mage::getStoreConfig('dgtyaris/product_page/zoom_position'):'right';
        $config['adjustX'] = is_numeric(Mage::getStoreConfig('dgtyaris/product_page/zoom_adjustX'))?Mage::getStoreConfig('dgtyaris/product_page/zoom_adjustX'):0;
        $config['adjustY'] = is_numeric(Mage::getStoreConfig('dgtyaris/product_page/zoom_adjustY'))?Mage::getStoreConfig('dgtyaris/product_page/zoom_adjustY'):0;
        if ($rel){
            $out = array();
