...
class DGTThemes_AjaxCart_IndexController extends Mage_Checkout_CartController
{
	/**
	* 
	...
	public function compareAction(){
		...
		if ($productId = (int) $this->getRequest()->getParam('product')) {
			...
			if ($product->getId()/* && !$product->isSuper()*/) {
				...
				$response['status'] = 'SUCCESS';
                $message = array();
                $message[] = '<p>'.$this->__('The product %s has been added to comparison list.', Mage::helper('core')->escapeHtml($product->getName())).'</p>';
                $message[] = '<img src="'.$product->getImageUrl().'" >';

                $message[] = '<div class="ajax-action"><a class="view-compare" onclick="popWin(\''.Mage::helper('catalog/product_compare')->getListUrl().'\',\'compare\',\'top:0,left:0,width=820,height=600,resizable=yes,scrollbars=yes\')"><span>'.$this->__('Compare') .'</span></a></div>';
